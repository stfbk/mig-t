...
public interface ExecuteTrackListener {
    ...
    /**
     * Called when the execution of the track is finished, and you want to force
     * the result of the test
     *
     * @param forceResult the final result the test will have, independently of
     *                    correct or incorrect flow
     * @param sessionName The name of the session which was linked to the execution
     */
    void onExecuteDone(boolean forceResult, String sessionName);

    /**
     * Called when the Browser crashes or some error is present, it is different
     * from the onExecuteDone having the error parameter set to true, this method
     * is called when an error not related to the test, and that does not have to
     * influence the result of the test is found.
     */
